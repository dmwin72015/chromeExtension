...
function loadI18nMessages() {
    ...
    document.querySelector('.progress').style.minWidth = getTextWidth(formatBytes(1024 * 1024 * 1023.9) + '/' + formatBytes(1024 * 1024 * 1023.9)) + 'px';
...
function formatBytes(n) {
    ...
    for (var i = 0; i < prefixes.length; ++i) {
        if (n < (1024 * mul)) {
            return (parseInt(n / mul) + '.' + parseInt(100 * ((n / mul) % 1)) + prefixes[i] + 'B');
        }
        mul *= 1024;
    }
    return '!!!';
...
function DownloadItem(data) {
    ...
    if (
        ...
        var adjacent_div = items_div.childNodes[binarySearch(arrayFrom(items_div.childNodes), item.startTime.getTime(),
    ...
    item.more_mousemove = function (evt) {
        ...
        if (item.getElement('more') && (pointInElement(mouse, item.div) || pointInElement(mouse, item.getElement('more')))) {
            return;
        }
...
DownloadItem.prototype.render = function () {
    ...
    item.deleted || !chrome.downloads.removeFile);
